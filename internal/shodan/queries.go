...
// BuildScopedQuery wraps the raw dork with any configured scope filter.
//
// Shodan operator precedence is AND-first, so `(scope) AND (A|B)` parses as
// `(scope AND A) OR B` — the OR clause escapes the scope entirely.  We
// prevent this by converting any OR-alternation in Raw to explicit double-
// parens before combining with the scope:
//
//	(org:"Acme") AND ((http.html:"open-webui") OR (http.title:"Open WebUI"))
//
// The raw dork strings in AllQueries() already use the canonical
// `((A) OR (B))` double-paren form, so this function simply wraps the
// whole thing in `(scope) AND (raw)` when a scope is present.
func (c Config) BuildScopedQuery(raw string) string {
	...
	// Explicit AND with both sides parenthesized so OR-alternation inside
	// raw cannot escape the scope constraint.
